...
ARaiderCharacter::ARaiderCharacter()
{
	...
	CollisionResponse.SetAllChannels(ECR_Ignore); // Игнорируем все каналы по умолчанию
	CollisionResponse.SetResponse(ECC_GameTraceChannel1, ECR_Block); // Ваш кастомный канал
...
{
   FVector Start = Camera->GetComponentLocation(); // Начальная позиция трассировки
   FVector End = Start + (Camera->GetForwardVector() * 300.f); // Конечная позиция
   float Radius = 2.0f; // Радиус сферы
   FCollisionQueryParams CollisionParams;
   CollisionParams.AddIgnoredActor(this); // Игнорируем самого себя
   ...
   AActor* HitActor = HitResult.GetActor(); // Получаем актера
   ...
   if (bHit && HitActor && HitActor->Implements<UInteractInterface>())
   {
      ...
      if (InteractInterfaceActor)
      {
         ...
         else
         {
            ...
            InteractActor = HitActor; // Обновляем ссылку на активного актора
            InteractInterfaceActor->ActiveOutline(true);
         }
      }
   }
   else
   {
      if (InteractActor)
      {
         ...
         InteractActor = nullptr; // Сбрасываем ссылку, если актора нет
         Color = FColor::Red;
      }
   }
